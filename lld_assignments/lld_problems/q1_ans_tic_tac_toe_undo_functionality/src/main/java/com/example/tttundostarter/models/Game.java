...
import com.example.tttundostarter.exceptions.GameInvalidationException;
import com.example.tttundostarter.exceptions.InvalidMoveException;
import com.example.tttundostarter.strategies.WinningStrategy;
import com.example.tttundostarter.strategies.WinningStrategyImpl;
...
public class Game {
    ...
    public void undo() throws EmptyMovesException {
        ...
        if (this.moves.size() == 0) {
            // Handle Edge Case
            throw new EmptyMovesException("Undo operation was called when no move existed");
        }

        Move lastMove = this.moves.get(this.moves.size() - 1);
        Cell relevantCell = lastMove.getCell();
        relevantCell.setCellState(CellState.EMPTY);
        this.nextPlayerMoveIndex -= 1;
        this.nextPlayerMoveIndex = (this.nextPlayerMoveIndex + this.players.size()) % this.players.size();
